...
temporary file under `docs/runs/code-runs`. This avoids `cmd.exe` argument
splitting that can truncate prompts containing spaces.

## Progress

Stdout and stderr are captured as JSONL events in `docs/runs/code-runs/{taskId}.jsonl`.
